...
-- The database has 'PENDING' and 'RESOLVED' (uppercase) but the function was looking for 'pending' and 'resolved' (lowercase)
...
BEGIN
    ...
    -- Count prompts sent (consultant actions)
    SELECT COUNT(*)
    INTO prompts_sent_count
    FROM public.consultant_actions
    WHERE consultant_id = p_consultant_id AND action_type = 'PROMPT_SENT';
