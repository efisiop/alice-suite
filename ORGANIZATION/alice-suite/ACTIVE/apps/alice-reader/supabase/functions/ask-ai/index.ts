...
serve(async (req: Request) => {
  ...
  try {
    ...
    console.log(`Sending request to Moonshot API (${mode} mode)`);
    ...
    if (!openAIResponse.ok) {
      console.error('Moonshot API error:', openAIData);
      throw new Error(openAIData.error?.message || 'Error calling Moonshot API');
    }
