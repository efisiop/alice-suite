...
getConsultantAssignments: async (): Promise<any[]> => {
  try {
    ...
    if (!sessionData.session?.user) {
      appLog(SERVICE_NAME, 'No authenticated user found for assignments', 'warning');
      return [];
    }
    ...
    const { data: assignments, error } = await supabase
      ...
      .select('*, user:user_id(id, first_name, last_name, email, last_active_at, book_verified, is_verified)')
      .eq('consultant_id', consultantId)
      .eq('active', true);

    if (error) {
      appLog(SERVICE_NAME, 'Error fetching consultant assignments', 'error', error);
      return [];
    }

    appLog(SERVICE_NAME, `Found ${assignments.length} consultant assignments`, 'success');
    return assignments || [];
  } catch (error) {
