...
import { Database, BookWithChapters, SectionWithChapter } from '@alice-suite/api-client';
...
// Async getter for the Supabase client - create only once
export const getSupabaseClient = async (): Promise<SupabaseClient<Database>> => {
  try {
    // Create client only once
    if (!supabaseClient) {
      appLog('SupabaseClient', 'Initializing Supabase client (singleton)', 'debug');
      ...
      appLog('SupabaseClient', 'Supabase client initialized successfully', 'success');
    }
