...
const ConsultantDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      const data = await dataServiceManager.getDashboardData(user.id);
      
      // Transform readers data to match expected format
      const transformedReaders = data.readers.map(reader => ({
        id: reader.id,
        name: `${reader.first_name} ${reader.last_name}`,
        progress: reader.progress || 0, // Use actual progress from data
        lastActive: reader.status === 'active' ? 'just now' : 'inactive',
        status: reader.status
      }));

      setDashboardData({
        assignedReaders: transformedReaders || [],
        helpRequests: data.helpRequests || [],
        feedback: data.feedback || [],
        currentlyLoggedIn: onlineReaders?.length || 0,
        stats: {
          totalReaders: data.analytics?.totalReaders || 0,
          activeReaders: data.analytics?.activeReaders || 0,
          pendingHelpRequests: data.analytics?.pendingHelpRequests || 0,
          resolvedHelpRequests: (data.helpRequests || []).filter(h => h.status === 'RESOLVED').length,
          feedbackCount: data.feedback?.length || 0,
          promptsSent: data.triggers?.length || 0
        }
      });
  ...
  // WebSocket functionality is now handled by useRealtimeAuth hook - removing redundant implementation
