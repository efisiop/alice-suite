...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const signOut = async () => {
    ...
    try {
      ...
      // Then, sign out from the auth service
      const { error } = await authSignOut();
      if (error) {
        appLog('AuthContext', 'Error during auth sign out', 'error', error);
        // Don't throw, as we want to complete the sign-out process client-side regardless
      } else {
        appLog('AuthContext', 'Auth sign out successful.', 'success');
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const setupAuthStateListener = async () => {
      try {
        const authStateResult = await authOnAuthStateChange((_event, session) => {
          if (mounted) {
  ...
  const signIn = async (email: string, password: string) => {
    ...
    try {
      ...
      // Set user immediately to prevent loading states
      setUser(data.user);
  ...
  const signUp = async (email: string, password: string, firstName: string, lastName: string) => {
    ...
    try {
      ...
      try {
        ...
        if (updateError) {
          appLog('AuthContext', `Warning: Could not update profile with names: ${String(updateError)}`, 'warning');
