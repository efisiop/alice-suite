...
export class EventBroadcaster {
  ...
  private async sendInitialConsultantData(socket: AuthenticatedSocket): Promise<void> {
    try {
  ...
  async updateOnlineReaders(): Promise<void> {
    try {
      const onlineUsers = await this.redis.getOnlineUsers();
      const onlineReaders = [];
  ...
  async broadcastToConsultants(event: string, data: any): Promise<void> {
    this.io.to('consultants').emit(event, data);
  }

  async broadcastToSupport(event: string, data: any): Promise<void> {
